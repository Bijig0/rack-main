...
const getPushEventScript = ({ tag, appId }: { tag: Tag; appId: string }) => {
  ...
  return {
    ...
    content: tag.pushEventScript?.content?.replace(
      '$pushEvent',
      `${PushEventPrefix}_${appId}`,
    ),
  }
}

function getAnalyticsApps(
  eventType: Parameters<typeof getEventTypeAppData>[0],
) {
  return Object.entries(appStoreMetadata).reduce(
    (acc, entry) => {
      const [appId, app] = entry
      const eventTypeAppData = getEventTypeAppData(
        eventType,
...
export default function BookingPageTagManager({
  ...
  return (
    <>
      {Object.entries(analyticsApps).map(
        ([appId, { meta: app, eventTypeAppData }]) => {
          ...
          return tag.scripts
            ...
            .map((script, index) => {
              const parsedAttributes: NonNullable<
                (typeof tag.scripts)[number]['attrs']
              > = {}
