...
export function useCheckOverlapWithOverlay({
  ...
  const isOverlapping =
    ...
    overlayBusyDates.some((busyDate) => {
      ...
      const selectedEndTime = dayjs(start.add(offset, 'hours')).add(
        selectedDuration ?? 0,
        'minute',
      )

      const isOverlapping =
        (selectedEndTime.isSame(busyDateStart) ||
          selectedEndTime.isAfter(busyDateStart)) &&
        start.add(offset, 'hours') < busyDateEnd &&
        selectedEndTime > busyDateStart

      overlappingTimeStart = isOverlapping
        ? getCurrentTime(busyDateStart.toDate())
        : null
      overlappingTimeEnd = isOverlapping
        ? getCurrentTime(busyDateEnd.toDate())
        : null
