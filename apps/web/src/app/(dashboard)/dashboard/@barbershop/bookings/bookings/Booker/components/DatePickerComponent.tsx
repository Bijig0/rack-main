...
import { weekdayNames } from '../utils/weekday'
import { daysInMonth, yyyymmdd } from './date/date-fns'
import { getAvailableDatesInMonth } from './date/getAvailableDatesInMonth'
import { useEmbedStyles } from './embed/embed-iframe'
...
const NoAvailabilityOverlay = ({
  ...
  return (
    <div className="absolute p-8 -mt-10 transform -translate-x-1/2 -translate-y-1/2 border rounded-md shadow-sm border-subtle left-1/2 top-40 w-max bg-muted">
      <h4 className="mb-4 font-medium text-emphasis">
        {'No availability in known month'}
      </h4>
      <Button
...
const ReschedulingNotPossibleOverlay = () => {
  ...
  return (
    <div className="absolute w-2/3 p-8 -mt-10 break-all transform -translate-x-1/2 -translate-y-1/2 border rounded-md shadow-sm border-subtle left-1/2 top-40 bg-muted">
      <h4 className="font-medium text-emphasis">
...
const Days = ({
  ...
  const [selectedDatesAndTimes] = useBookerStore(
  ...
  const daysToRenderForTheMonth = days.map((day) => {
    ...
    const oooInfo =
      slots && slots?.[dateKey]
        ? slots?.[dateKey]?.find((slot) => slot.away)
        : null
    ...
    const isOOOAllDay = !!(
      slots &&
      slots[dateKey] &&
      slots[dateKey].every((slot) => slot.away)
    )
      ...
      {daysToRenderForTheMonth.map(({ day, disabled, away, emoji }, idx) => (
        ...
        >
          {day === null ? (
            ...
            <button
              className="absolute top-0 bottom-0 left-0 right-0 flex items-center justify-center w-full mx-auto font-medium text-center transition border-transparent rounded-sm opacity-50 bg-muted text-muted"
              key={`e-${idx}`}
              disabled
            >
  ...
  return (
    <div className={className}>
      <div className="flex items-center justify-between mb-1 text-xl">
        <span className="w-1/2 text-base text-default">
          {browsingDate ? (
            <>
              <strong
                className={classNames(
      ...
      <div className="grid grid-cols-7 gap-4 mb-2 text-center border-t border-b border-subtle md:mb-0 md:border-0">
