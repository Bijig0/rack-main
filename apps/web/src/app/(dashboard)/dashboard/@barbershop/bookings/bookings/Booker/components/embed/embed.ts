...
import { EventData, EventDataMap, SdkActionManager } from './sdk-action-manager'
...
function validate(data: Record<string, unknown>, schema: ValidationSchema) {
  ...
  for (const [prop, propSchema] of Object.entries(schema.props || {})) {
    ...
    if (!typeCheck) {
      throw new Error(
        `"${prop}" is of wrong type.Expected type "${propSchema.type}"`,
      )
    }
  }
}
...
type SingleInstructionMap = {
  ...
  [K in keyof CalApi]: CalApi[K] extends (...args: never[]) => void
...
class CalApi {
   ...
   */
  init(
    namespaceOrConfig?: string | InitArgConfig,
    config = {} as InitArgConfig,
  ) {
    ...
    this.cal.__config.calOrigin =
      calOrigin || origin || this.cal.__config.calOrigin
  ...
  initNamespace(namespace: string) {
    ...
    globalCal.ns[namespace].instance =
      globalCal.ns[namespace].instance ||
      new Cal(namespace, globalCal.ns[namespace].q)
  }
  /**
  ...
  inline({
    ...
    config = config || {}
    if (
      typeof config.iframeAttrs === 'string' ||
      config.iframeAttrs instanceof Array
    ) {
  ...
  modal({
    ...
    const uid =
      this.modalUid || this.preloadedModalUid || String(Date.now()) || '0'
    const isConnectingToPreloadedModal =
      this.preloadedModalUid && !this.modalUid
    ...
    const existingModalEl = document.querySelector(
      `cal-modal-box[uid="${uid}"]`,
    )
  ...
  preload({
    ...
    if (prerenderIframe) {
      if (type === 'modal' || type === 'floatingButton') {
        ...
        console.warn(
          'Ignoring - full preload for inline embed and instead preloading assets only',
        )
...
export interface GlobalCalWithoutNs {
  <T extends keyof SingleInstructionMap>(
    methodName: T,
    ...arg: Rest<SingleInstructionMap[T]>
  ): void
...
document.addEventListener('click', (e) => {
  ...
  function getCalLinkEl(target: EventTarget | null) {
    ...
    if (target?.dataset.calLink) {
      ...
      calLinkEl = Array.from(document.querySelectorAll('[data-cal-link]')).find(
...
function preloadAssetsForCalLink({
  config,
  calLink,
}: {
  config: Config
