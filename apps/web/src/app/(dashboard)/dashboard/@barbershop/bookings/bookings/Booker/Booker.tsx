...
import { BookerLayouts } from './types'
...
import { UnpublishedEntity } from './components/UnpublishedEntity'
import {
  fadeInLeft,
...
const BookerComponent = ({
  ...
  const [dayCount, setDayCount] = useBookerStore(
    (state) => [state.dayCount, state.setDayCount],
    shallow,
  )

  const nonEmptyScheduleDays = useNonEmptyScheduleDays(
    schedule?.data?.slots,
  ).filter((slot) => dayjs(selectedDate).diff(slot, 'day') <= 0)
  ...
  return (
    <>
      {/* {event.data && !isPlatform ? (
        <BookingPageTagManager eventType={event.data} />
      ) : (
        <></>
      )} */}
      ...
      >
        <div
          ...
          className={classNames(
            ...
            `bg-default grid max-w-full items-start dark:bg-muted dark:[color-scheme:dark] sm:transition-[width] sm:duration-300 sm:motion-reduce:transition-none md:flex-row`,
        ...
        >
          <AnimatePresence>
              ...
              <BookerSection
                ...
                className={classNames(
                  ...
                  (layout === BookerLayouts.COLUMN_VIEW ||
                    ...
                    'bg-default sticky top-0 z-10 dark:bg-muted',
                )}
              >
                {/* {!isPlatform ? (
                  <Header
            ...
            <BookerSection
              ...
              area="main"
              visible={
                bookerState !== 'booking' && layout === BookerLayouts.MONTH_VIEW
              }
              {...fadeInLeft}
              initial="visible"
