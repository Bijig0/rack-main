...
type setNonStylesConfig = React.Dispatch<
  React.SetStateAction<EmbedNonStylesConfig>
>
...
const embedStore = {
  ...
  reactNonStylesStateSetters: {} as Record<
    keyof EmbedNonStylesConfig,
    setNonStylesConfig
  >,
...
function log(...args: unknown[]) {
  if (isBrowser) {
    ...
    const logQueue = (window.CalEmbed.__logQueue =
      window.CalEmbed.__logQueue || [])
    args.push({
      ns: namespace,
      url: document.URL,
    })
