...
import { BookerLayouts, type BookerLayout, type BookerState } from './types'
...
export const getBookerSizeClassNames = (
  ...
  return [
    ...
    layout === BookerLayouts.MONTH_VIEW &&
      getBookerMetaClass('[--booker-meta-width:240px]'),
 ...
 */
export const useBookerResizeAnimation = (
  layout: BookerLayout,
  state: BookerState,
) => {
  ...
  useEffect(() => {
    ...
    const animationConfig =
      // resizeAnimationConfig[layout][state] ||
      resizeAnimationConfig[layout].default
      ...
      Object.keys(styles).forEach((property) => {
        if (property === 'height') {
          ...
          animationScope.current.style.height =
            animatedProperties.height === '100vh' && isEmbed
              ? '100%'
