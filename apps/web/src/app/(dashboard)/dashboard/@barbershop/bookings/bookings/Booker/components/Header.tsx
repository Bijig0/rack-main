...
import { BookerLayouts, type BookerLayout } from '../types'
import { useIsEmbed } from './embed/embed-iframe'
...
export function Header({
  ...
  const isEmbed = true
  const [layout, setLayout] = useBookerStore(
  ...
  const endDate = selectedDate.add(
    layout === BookerLayouts.COLUMN_VIEW ? extraDays : extraDays - 1,
    'days',
  )
      ...
      <div className="flex items-center gap-5 rtl:flex-grow">
        ...
        <ButtonGroup>
          <Button
            ...
            onClick={() =>
              addToSelectedDate(
                layout === BookerLayouts.COLUMN_VIEW ? -nextSlots : -extraDays,
              )
            }
          />
          <Button
...
const LayoutToggle = ({
  ...
  const layoutOptions = useMemo(() => {
    return [
      ...
      enabledLayouts?.includes(layout.value as BookerLayouts),
    )
  }, [t, enabledLayouts])
