...
export function OverlayCalendarSwitch({
  enabled,
  hasSession,
  onStateChange,
}: OverlayCalendarSwitchProps) {
  const { t } = useLocale()
  const setContinueWithProvider = useOverlayCalendarStore(
    (state) => state.setContinueWithProviderModal,
  )
  ...
  return (
    <div
      className={classNames(
        'hidden gap-2',
        layout === 'week_view' || layout === 'column_view'
          ? 'xl:flex'
          : 'md:flex',
      )}
    >
      <div className="flex items-center gap-2 pr-2">
        ...
        <label
          ...
          className="text-sm font-medium leading-none text-emphasis hover:cursor-pointer"
        >
