...
const sortByDate = (
  a: { ranges: TimeRange[]; id: string },
  b: { ranges: TimeRange[]; id: string },
) => {
...
const DateOverrideList = ({
  ...
  const timeSpan = ({ start, end }: TimeRange) => {
    return `${new Intl.DateTimeFormat('en', {
      ...
      minute: 'numeric',
      hour12,
    }).format(new Date(end.toISOString().slice(0, -1)))}`
  }

  return (
    <ul
      className="border-subtle rounded border"
      data-testid="date-overrides-list"
    >
      {fields.sort(sortByDate).map((item) => (
        ...
        >
          <div>
            ...
            {item.ranges[0].start.valueOf() - item.ranges[0].end.valueOf() ===
            0 ? (
                ...
                <p key={i} className="text-subtle text-xs">
                  {`${timeSpan(range)} ${
                    travelSchedules
                      .find(
                        (travelSchedule) =>
                          !dayjs(item.ranges[0].start).isBefore(
                            travelSchedule.startDate,
                          ) &&
                          (!dayjs(item.ranges[0].end).isAfter(
          ...
          <div className="flex flex-row-reverse gap-5 space-x-2 rtl:space-x-reverse">
            <DateOverrideInputDialog
              ...
              onChange={(ranges) => {
                ...
                replace([
                  ...fields.filter((currentItem) => currentItem.id !== item.id),
                  { ranges },
                ])
