...
import { yyyymmdd } from '../../components/date/date-fns'
...
const DateOverrideForm = ({
  ...
  const onDateChange = (newDate: Dayjs) => {
    ...
    if (selectedDates.some((date) => yyyymmdd(date) === yyyymmdd(newDate))) {
      setSelectedDates(
        selectedDates.filter((date) => yyyymmdd(date) !== yyyymmdd(newDate)),
      )
      return
    }
  ...
  const defaultRanges = (workingHours || []).reduce(
    (dayRanges: TimeRange[], workingHour) => {
      if (
        selectedDates[0] &&
        workingHour.days.includes(selectedDates[0].day())
      ) {
        dayRanges.push({
          start: dayjs
            .utc()
            .startOf('day')
            .add(workingHour.startTime, 'minute')
            .toDate(),
          end: dayjs
            .utc()
            .startOf('day')
            .add(workingHour.endTime, 'minute')
            .toDate(),
        })
      }
      return dayRanges
        ...
        {selectedDates[0] ? (
          <>
            <div className="mb-4 flex-grow space-y-4">
              <p className="text-medium text-emphasis text-sm">
