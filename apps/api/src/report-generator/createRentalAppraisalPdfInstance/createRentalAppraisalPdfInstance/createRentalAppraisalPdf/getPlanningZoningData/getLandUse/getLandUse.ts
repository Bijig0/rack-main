 ...
 * @returns Effect containing the land use category wrapped in Option
 */
export const getLandUse = ({
  address,
}: Args): Effect.Effect<LandUseData, Error> =>
  Effect.gen(function* () {
    const { planningZoneData } = yield* getPlanningZoneData({ address });

    const zoneCode = O.flatMap(planningZoneData, (data) =>
      data?.zoneCode ? O.some(data.zoneCode) : O.none()
    );

    if (O.isNone(zoneCode)) {
      return { landUse: O.none() };
    }

    const landUse = getLandUseFromZoneCode(O.getOrThrow(zoneCode));
    return { landUse: landUse ? O.some(landUse) : O.none() };
  });
...
if (import.meta.main) {
  ...
  for (const test of testAddresses) {
    const { landUse } = await Effect.runPromise(
      getLandUse({ address: test.address })
    );
    console.log(
      `${test.name}: ${O.getOrElse(landUse, () => "Not found")}`
    );
  }
}
